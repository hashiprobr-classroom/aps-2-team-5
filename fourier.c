...
void nft_forward(complex s[], complex t[], int n) {nft(s,t,n,-1);}
...
void fft_forward_2d(complex matrix[MAX_SIZE][MAX_SIZE], int width, int height) {
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {  s[x] = matrix[y][x];}
        fft_forward(s, t, width);
        for (int x = 0; x < width; x++) {  matrix[y][x] = t[x];
        }
    }
    for (int x = 0; x < width; x++) {
        for (int y = 0; y < height; y++) { s[y] = matrix[y][x] ;  }
        fft_forward(s, t, height);
        for (int y = 0; y < height; y++) {  matrix[y][x] = t[y];
        }
    }}
